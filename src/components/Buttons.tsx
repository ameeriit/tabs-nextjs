...
function TabsButton({ buttonText }: TabsButtonProps) {
  return (
    <button className="relative z-10 inline-block rounded-bl-lg group font-base font-medium">
      <div className="relative bg-primary-200 px-6 rounded-bl-[inherit] py-4 rounded-tr-4xl transition-all duration-300 ease-in-out group-hover:-translate-y-2">
        <span className="flex items-center gap-3 ">
          {buttonText} <DownArrowIcon />
        </span>
      </div>
      <span className="absolute h-full left-0 bottom-0 bg-shadow rounded-bl-[inherit] -z-10 rounded-tr-5xl group-hover:w-[calc(100%+8px)] w-full transition-all duration-300 ease-in-out" />
    </button>
  );
}
